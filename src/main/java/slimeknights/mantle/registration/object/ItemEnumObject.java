...
public class ItemEnumObject<T extends Enum<T>, I extends Item> {
  /** Singleton empty object, type does not matter as it has no items */
  private static final ItemEnumObject EMPTY = new ItemEnumObject(Collections.emptyMap());

  /** Internal backing supplier map */
  private final Map<T,Supplier<? extends I>> map;

  public ItemEnumObject(Map<T, Supplier<? extends I>> map) {
    this.map = map;
  }
  ...
  /**
   * Gets a list of values in this enum object. Will error if a {@link DefaultedRegistry} cannot be resolved, unlike {@link #forEach(Consumer)}
  ...
  public static <T extends Enum<T>, I extends Item> ItemEnumObject<T,I> empty() {
    return (ItemEnumObject<T,I>) EMPTY;
  }
  ...
  public static class Builder<T extends Enum<T>, I extends Item> {
    private final Map<T, Supplier<? extends I>> map;
     ...
     */
    @SuppressWarnings("unchecked")
    public Builder<T,I> putDelegate(T key, Item value) {
     ...
     * @return  Builder instance
     */
    public Builder<T,I> putAll(ItemEnumObject<T,? extends I> object) {
     ...
     */
    public ItemEnumObject<T,I> build() {
      return new ItemEnumObject<>(map);
    }
  }
}